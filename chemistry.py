...
class Chemistry(object):
    ...
    def index_of_molecule(self,mol):
        return( list(self.molecules.keys()).index(mol.atom_string) )
    ...
    def set_conc_for_all_testtubes(self,molecule : Molecule, concentration):
        for tt in self.testtubes:
            tt.concentrations[molecule] = concentration        
    ...
    def kinetics_rhs(self,m,latex=False):
        ...
        for rxn_i,r in enumerate(self.reactions):
            if latex == False:
                rxn_i = None
            if self.a_constituent_in_this_reaction_exists(r) :
                #print(r)
                for _ in range(r.lhs_count(m)):
                    s+= '-'+r.forward_kinetic_term(rxn_i=rxn_i)
                    if r.kb != 0.0:
                        s+= '+'+r.backward_kinetic_term(rxn_i=rxn_i)
    ...
    def write_eqs_as_latex(self,fn):
        # s = r'\begin{table}'+'\n'
        s = ''
        s += r'\begin{tabular}{r|rlcc}'+'\n'
        s += r'Reaction \# & & & $ \phi $ & $ \beta $ \\' + '\n'
        s += r'\hline'
        for r_i,r in enumerate(self.reactions) :
            s += f'{r_i} & '+r.latex()
        s += r'\end{tabular}'+'\n'
        # s += r'\end{table}'+'\n'
        with open(fn, "wt") as text_file:
            text_file.write(s)


    def write_mol_table_as_latex(self,fn):
        s = ''
        # s += r'\begin{table}'+'\n'
        s += r'\begin{tabular}{r|cccc}'+'\n'
        s += r' & $D_v$ & $\gamma_s$ & $H_v$ & $\chi_v$ \\' + '\n'
        s+= r'\hline'
...
{{derivs}}
    ...
    def write_ode_latex(self,fn):
        ...
        for m in self.molecules.values() :
            ...
            s += f'{m.atom_string}'
            s += '}) & = '
            rhs = str( self.kinetics_rhs(m,latex=True) )            
            s+= rhs.replace('**','^').replace('*',' ')
            ss.append(s)
        full_s = '\\\\ \n'.join(ss)
        
        
        with open(fn, "wt") as text_file:
            text_file.write(full_s)
    ...
    def dcdt(self, testtube, dt, ode_fn=None):
        ...
        # sol = odeint(ode_fn, y0, np.linspace(0,dt,10),full_output=False)
        # # n = np.shape(sol)[1]
        # # print(info)

        # change = sol[-1,:] - sol[0,:]
        # # ## update concentrations (change)
        # # for i,m in enumerate(self.molecules.values()):
        # #     testtube.concentrations[m] = sol[-1,i]

        change_direct = np.array([x for x in self.ode_fn(y0,0)])
        # print()
        # print(change)1
        # print(change_direct)
        # print(max(abs(np.array([a-b for a,b in zip(change,change_direct)]))))

        # ## update concentrations (change_direct)
        # for i,m in enumerate(self.molecules.values()):
        #     testtube.concentrations[m] += change_direct[i]*0.01#sol[-1,i]
        
        return change_direct
